...
const EditProduct = ({ onClose, fetchAllProducts, productData }) => {

    const [product, setProduct] = useState({ ...productData }) //change
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        const apiData = await fetch(SummaryApi.updateProduct.url, {  //change
        ...
        <div className='flex fixed w-full h-full bg-opacity-50 bg-slate-300 top-0 z-50 bottom-0 left-0 right-0 justify-center items-center px-3 md:px-0'>
            <div className='bg-white dark:bg-gray-700 w-full max-w-xl h-full max-h-[72%] overflow-y-scroll scrollbar-none px-5 py-4 rounded-md'>
                <div className='flex justify-between items-center pb-3'>
                    <h1 className='font-bold dark:text-white' style={{ fontSize: "20px" }}>Edit Product</h1>
                    <div
                ...
                <form className='grid gap-2' onSubmit={handleSubmit}>
                    ...
                    <input
                        ...
                        name="numberOfStock"
                        id="numberOfStock"
                        value={product.stockManagement == "true" || product.stockManagement == true ? product.numberOfStock || "" : 0}
                        onChange={handleOnChange}
                        disabled={product.stockManagement == "true" || product.stockManagement == true ? false : true}
                        className='p-2 border rounded-md dark:bg-gray-700 dark:border-white-600 dark:text-white'
                    />
                    <button className='bg-blue-500 text-white py-3 rounded-md'>Update Product</button>
                </form>
            </div>
        </div>
    )
